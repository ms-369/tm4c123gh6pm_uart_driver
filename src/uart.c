 ...
 * @return uart_Regs* 
 */
static inline uart_Regs* uart_getBase(uart_Number uart_number)
{
    switch (uart_number)
    {
        case UART_0:
...
uart_Error uart_init(uart_Config* config)
{
    ...
    if (config->BAUD_RATE < BAUD_RATE_9600 || config->BAUD_RATE > BAUD_RATE_115200) {
    ...
    if (config->UART_NUM == UART_0) {
        ...
        uart_pin.PCTL_VAL = UART_PIN_MUX;
 ...
 * @return uart_Error
 */
uart_Error uart_sendChar(uart_Config* config, char data)
{
    uart_Regs *uart = uart_getBase(config->UART_NUM);
    if (!uart) {
 ...
 * @return uart_Error
 */
uart_Error uart_sendString(uart_Config* config, char* data)
{
 ...
 * @return uart_Error
 */
uart_Error uart_receiveChar(uart_Config* config, char* data)
{
    uart_Regs *uart = uart_getBase(config->UART_NUM);
    if (!uart) return UART_NOT_CONFIGURED;
    ...
    *data = (char)(uart->UARTDR & MAX_8_BIT);
...
void uart0_interruptCallback(void)
{
    ...
    rx = (char)(UART0->UARTDR & MAX_8_BIT);
...
void uart1_interruptCallback(void)
{
    ...
    rx = (char)(UART1->UARTDR & MAX_8_BIT);
    /* In case of 'Enter' enter new line instead of carriage return */
    if (rx == '\r') {
        UART1->UARTDR = '\r';
        UART1->UARTDR = '\n';
